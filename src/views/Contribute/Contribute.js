...
const Contribute = () => {
  ...
  const onFormSubmit = (e) => {
    ...
    service
      .post(FORM_FILL_STRUCTURED, payload)
      .then((res) => {
        if (res.data.success) {
  ...
  const discardData = (e) => {
    e.preventDefault();
    service
      .delete(`${FORM_FILL_STRUCTURED}/${cardData.uuid}`)
      .then((res) => {
        if (res.data.success) {
          setCardData(null);
          setShowCard(false);
  ...
  return (
    <div className="layout-wrapper">
      <div className="contribute-container">
        <div className="tabs">
          <span
            onClick={() => setStructuredFormOpen(true)}
            className={structuredFormOpen ? "active" : null}
          >
            Add Info
          </span>
          <span
            onClick={() => setStructuredFormOpen(false)}
            className={!structuredFormOpen ? "active" : null}
          >
            Upload File
          </span>
        ...
        {structuredFormOpen ? (
          <div className="fill-form">
            <p>
              Please fill this form with complete honesty. We depend on the
              goodwill of the community, so please do not intentionally spam
              this form. If you cannot help, please do not hinder others
              intention to help.
            </p>
            <form onSubmit={onFormSubmit}>
              ...
              <div className="block">
                <FormControl>
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, isGiver: e.target.value })
                    }
                  >
              ...
              <div className="control-buttons">
                <button className="btn" onClick={onAddContact}>
                  Add New Contact
                </button>
          ...
          <div className="fill-form">
            <p>
              This feature is currently under construction. Thank you for your
              patience.
            </p>
          </div>
        )}
          ...
          <div className="card-preview">
            ...
            {cardData && cardData.uuid ? (
              <Card
                isLink={false}
                {...cardData}
                updateCard={(value) => setCardData(value)}
              />
            ) : null}
            <button className="btn" onClick={discardData}>
              Discard
            </button>
          </div>
        ) : null}
