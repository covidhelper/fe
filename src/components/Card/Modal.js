  ...
  return (
    props.open && (
      <div className="modal-wrapper">
        <div
          className={`modal ${props.type !== "confirm" ? "inc-height" : null}`}
        >
              ...
              {props.data.map((c, ind) => {
                ...
                return (
                  <div key={ind} className="comments">
                    <div className="date">{`Date: ${formatString(
                      date.getDate()
                    )}/${formatString(
                      date.getMonth() + 1
                    )}/${date.getFullYear()}, Time: ${formatString(
                      date.getHours()
                    )}:${formatString(date.getMinutes())}`}</div>
